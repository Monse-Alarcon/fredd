    ...
    <div class="py-8 max-w-7xl mx-auto">
        ...
        <div class="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="bg-white shadow-md rounded-lg p-4 border-l-4 border-blue-500">
                <div class="text-sm text-gray-600">Tickets Asignados</div>
        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="p-4 bg-pink-100 border-b">
                <h3 class="text-lg font-semibold text-pink-700">Tickets Asignados</h3>
            </div>
                            ...
                            <td class="py-2 px-4 border-b text-center">
                                <div class="flex gap-2 justify-center">
                                    ...
                                    @if($ticket->estado != 'cerrado')
                                        <form action="{{ route('tickets.update-status', $ticket->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('PATCH')
                                            <input type="hidden" name="estado" value="{{ $ticket->estado == 'abierto' ? 'en progreso' : 'cerrado' }}">
                                            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded shadow text-sm">
                                                {{ $ticket->estado == 'abierto' ? 'En Proceso' : 'Finalizar' }}
                                            </button>
                                        </form>
                                    @endif
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-8 text-center text-gray-500">
                                No tienes tickets asignados. El jefe debe asignarte tickets para que puedas verlos aquí.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
